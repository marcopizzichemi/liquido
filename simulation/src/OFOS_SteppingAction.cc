...
//  if (stepStatus == fAtRestDoItProc) return;
    ...
    {
        G4cout << "ANNIHILATION!!! (originated by pdgid " << theTrack->GetDefinition()->GetPDGEncoding() << G4endl;
    }
    ...
    if( OFOS_Verbosity::level>2 &&  pp_proc_type != G4ProcessType::fTransportation  && particleDefinition->GetPDGEncoding() != 0 )
  ...
  //     || (thePostPVname.data()[0] == 'B') ) /// Buffer
    {
        ...
        { 
            if(sec_tracks->at(i)->GetDefinition()->GetPDGEncoding() != 0)
            {
                ...
                list_of_secondaries += G4String( TString::Format("%d", sec_tracks->at(i)->GetTrackID()).Data() );
                list_of_secondaries += " ("; 
                list_of_secondaries += G4String( TString::Format("%d", sec_tracks->at(i)->GetDefinition()->GetPDGEncoding()).Data() );
                list_of_secondaries += ")  ";
                list_of_secondaries += "mom_id: ";
                list_of_secondaries += G4String( TString::Format("%d\n", sec_tracks->at(i)->GetParentID()).Data() );
            }
        }
