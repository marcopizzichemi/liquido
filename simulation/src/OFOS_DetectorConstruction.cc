...
OFOS_DetectorConstruction::ConstructSDandField()
{
 // Sensitive detectors
    ...
    {
        if(OFOS_Verbosity::level>1)
            OFOS_OutputLog::log_cache  << "OFOS_DetectorConstruction::ConstructSDandField() :: Setting Sensitive Detectors" << G4endl;
...
OFOS_DetectorConstruction::deploy_ls_properties()
{
    ...
    OFOS_OutputLog::log_cache << "LS Light Yield:                      " <<  my_ls_properties->get_light_yield()  << " / MeV"<< G4endl;
    ...
    if(OFOS_Verbosity::level>1)
    {
        ...
        OFOS_OutputLog::log_cache << "OFOS_DetectorConstruction::deploy_ls_properties() :: cocktail name: " << ls->GetName() << G4endl;
    }
 ...
 // if(ls_mpt) delete ls_mpt; /// if deleted causes seg fault -> G4 ownership?
    ...
    if(OFOS_Verbosity::level>1)
    {
        OFOS_OutputLog::log_cache << "OFOS_DetectorConstruction::deploy_ls_properties() :: LS properties updated" << G4endl;
    }
...
OFOS_DetectorConstruction::update_geom()
{
    ...
    for(iLV=0; iLV<nLV && OFOS_Verbosity::level>1; ++iLV)
    {
        ...
        if( pLV->GetName() == "LS")
        {
            OFOS_OutputLog::log_cache << "OFOS_DetectorConstruction::update_geom() :: LS Material : " << pLV->GetMaterial()->GetName() << G4endl;
...
OFOS_DetectorConstruction::build_geom()
{
    ...
    /// external world
    G4double world_x_size = outer_vessel_x_size + 1.0 * m ; //size_x_liquido;
    G4double world_y_size = outer_vessel_y_size + 1.0 * m ; //size_y_liquido;
    G4double world_z_size = outer_vessel_z_size + 1.0 * m ; //size_z_liquido;
   
   
    /// info dump
    OFOS_OutputLog::log_cache << "---------------------------------------" <<  G4endl;
    OFOS_OutputLog::log_cache << "***   geometry derived parameters   ***" << G4endl;
    OFOS_OutputLog::log_cache << "LS size (x,y,z): (" << ls_vessel_x_size/m << ", " << ls_vessel_y_size/m << ", " << ls_vessel_z_size/m << ") m" << G4endl;
    ...
    G4VPhysicalVolume* hori_ru_sipmt_bot_p         = new G4PVPlacement( G4Transform3D(rot_sipmt,
                                                      G4ThreeVector(0.,0.,-0.5 * hori_fiber_env_length + 0.5 * sipm_thickness)), hori_ru_sipmt_l, "SiPMT",
    ...
    for(int ix = 0 ; ix < number_ru_x_-1; ix++) 
    {
        for(int iz = 0 ; iz < number_ru_z_; iz++) 
        {
            ...
            OFOS_OutputLog::geom_cache << number_ru_x_*number_ru_y_ + iz + ix*number_ru_x_  << "\t" << xpos << "\t" << zpos << G4endl;
        }
    }
    ...
    G4VPhysicalVolume* vert_ru_sipmt_bot_p         = new G4PVPlacement( G4Transform3D(rot_sipmt,
                                                       G4ThreeVector(0.,0.,-0.5 * vert_fiber_env_length + 0.5 * sipm_thickness)), 
                                                       vert_ru_sipmt_l, "SiPMT", vert_fiber_env_l, false, 1, fCheckOverlaps);
